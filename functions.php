...
if( !function_exists("wp_bootstrap_theme_styles") ) {  
    function wp_bootstrap_theme_styles() { 
        ...
        // For child themes
        wp_register_style( 'wpbs-style', get_stylesheet_directory_uri() . '/style.css', array(), wp_get_theme()->get('Version') );
...
if( !function_exists( "wp_bootstrap_theme_js" ) ) {  
  function wp_bootstrap_theme_js(){
    ...
    wp_register_script( 'main-script', 
...
function wpf_dev_field_properties_222( $properties, $field, $form_data ) {
    if ( absint( $form_data['id'] ) !== 222 ) {
        return $properties;
    } {
        $properties['description']['position'] = 'before';
    }
...
add_filter( 'wpforms_field_properties', 'wpf_dev_field_properties_222', 10, 3 );
?>
